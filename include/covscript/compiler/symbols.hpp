...
#include <covscript/compiler/binary_tree.hpp>
#include <iostream>
...
namespace cs
{
	...
	{
		memory_manager::stack_visitor mId;
	   ...
	   public:
		...
		const memory_manager::stack_visitor &get_id() const noexcept
		{
			return this->mId;
		}
	...
	class token_expand final : public token_base
	{
	   ...
	   public:
		...
		explicit token_expand(binary_tree<token_base *> tree) : mTree(std::move(tree)) {}
		...
		binary_tree<token_base *> &get_tree() noexcept
		{
			return this->mTree;
		}
	...
	class token_expr final : public token_base
	{
	   ...
	   public:
		...
		explicit token_expr(binary_tree<token_base *> tree) : mTree(std::move(tree)) {}
	...
	{
		std::deque<binary_tree<token_base *>> mTreeList;

	   public:
		token_arglist() = default;

		explicit token_arglist(std::deque<binary_tree<token_base *>> tlist) : mTreeList(std::move(tlist))
		{
		}
		...
		std::deque<binary_tree<token_base *>> &get_arglist() noexcept
		{
	...
	class token_array final : public token_base
	{
	   ...
	   public:
		...
		explicit token_array(std::deque<binary_tree<token_base *>> tlist) : mTreeList(std::move(tlist))
		{
		}
		...
		std::deque<binary_tree<token_base *>> &get_array() noexcept
		{
			return this->mTreeList;
		}
	...
	class token_parallel final : public token_base
	{
	   ...
	   public:
		...
		explicit token_parallel(std::deque<binary_tree<token_base *>> tlist) : mTreeList(std::move(tlist))
		{
		}
		...
		std::deque<binary_tree<token_base *>> &get_parallel() noexcept
		{
			return this->mTreeList;
		}
